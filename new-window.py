...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        ...
        self.model = self.model_load(weights="runs2/train/exp_big/weights/best.pt",
    ...
    ***界面初始化***
    ...
    def initUI(self):
        ...
        up_img_button.setFont(font_title)
        ...
        self.setTabIcon(2, QIcon('images/UI/OIP.jpg'))
    ...
    ***检测图片***
    ...
    def detect_img(self):
        ...
        imgsz = [self.detect_size, self.detect_size]  # inference size (pixels)
        ...
        if source == "":
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    # 将检测结果还原到原始尺寸
                    resize_scale = orig_shape[0] / im0.shape[0]  # 计算原始尺寸比例
                    im0 = cv2.resize(im0, (orig_shape[1], orig_shape[0]))  # 还原到原始尺寸

                    # 调整显示尺寸（保持UI显示尺寸不变）
                    display_scale = output_size / im0.shape[0]
                    display_img = cv2.resize(im0, (0, 0), fx=display_scale, fy=display_scale)

                    cv2.imwrite("images/tmp/single_result.jpg", display_img)  # 保存显示用图片
                    self.right_img.setPixmap(QPixmap("images/tmp/single_result.jpg"))
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                annotator = Annotator(im0, line_width=line_thickness, example=str(names))
                if len(det):
                    # 将检测框坐标从320x320缩放到原始尺寸
                    det[:, :4] = scale_coords(im.shape[2:], det[:, :4], orig_shape).round()
                    ...
                    # 还原到原始尺寸
                im0 = annotator.result()
                im0 = cv2.resize(im0, (orig_shape[1], orig_shape[0]))  # 关键修改：恢复原始分辨率
                ...
                # 更新UI
                cv2.imwrite("images/tmp/single_result_vid.jpg", display_img)
                self.vid_img.setPixmap(QPixmap("images/tmp/single_result_vid.jpg"))

                if cv2.waitKey(1) == ord('q') or self.stopEvent.is_set():
                    self.stopEvent.clear()
